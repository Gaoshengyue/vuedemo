    ...
    <div>
      ...
      <div style="width: 980px;margin: 0 auto">
        ...
        <div v-show="tabs.detail">
          ...
          <div>
            <ul class="price-policy">
              <li v-bind:class="[{active:num==selectCourseIndex}]" v-on:click="choiceCourse(num)"
                  v-for="(pri,num) in prices"> {{num}} ¥{{pri.price}} (有效期 {{pri.period}} )
              </li>
            </ul>
          ...
          <p>推荐课程</p>
          <ul>
            <li style="cursor: pointer" v-for="row in detail.recommends" @click="recommendCourse(row.id)">{{row.name}}</li>
          </ul>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      buyCar: function () {
        ...
        if (!token) {
          this.$router.push({name: 'login', query: {backrl: this.$route.path}})
          return
        }
